...
<script setup lang="ts" name="TotalTendency">
  ...
  const option = reactive({
    ...
    legend: {
      ...
      top: 30,
      selected: ActionsEnum.reduce((acc: any, name, index) => {
        // 默认只显示异常行为，不显示正常行为
        acc[name] = index !== 0;
        return acc;
      }, {})
  ...
  watch(
    ...
    () => {
      ...
      // 初始化每种行为的数据数组
      ActionsEnum.forEach((action, index) => {
      ...
      getTimePoints.value.forEach((timePoint, timeIndex) => {
        ...
        stats.forEach((count, actionIndex) => {
          if (actionIndex < ActionsEnum.length) {
            seriesData[ActionsEnum[actionIndex]][timeIndex] = count;
          }
        });
      });
      // 创建系列
      option.series = ActionsEnum.map((action, index) => {
        // 计算该行为的总数
