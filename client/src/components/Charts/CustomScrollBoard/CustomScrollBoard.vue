<template>
  <div class="custom-scroll-board">
    ...
    <div class="board-header" :style="{ background: headerBGC }">
      <div v-for="(item, index) in config.header" :key="index" class="header-item" :style="{
        width: columnWidthStyle(index),
    ...
    <div class="board-body" ref="boardBody">
      <transition-group name="scroll-list" tag="div" class="scroll-items"
        :style="{ transform: `translateY(${offset}px)` }">
        <div v-for="(row, rowIndex) in visibleData" :key="row.key || rowIndex" class="body-row"
          :class="{ odd: rowIndex % 2 === 0 }" :style="{
            height: `${rowHeight}px`,
            lineHeight: `${rowHeight}px`,
            backgroundColor: rowIndex % 2 === 0 ? oddRowBGC : evenRowBGC
          }">
          <div v-if="config.index" class="index-column" :style="{
            width: '50px',
            textAlign: 'center'
          }">
            {{ rowIndex + 1 }}
          </div>
          <div v-for="(item, colIndex) in row.data" :key="colIndex" class="body-item" :style="{
...
const startScroll = () => {
  ...
  scrollTimer = window.setInterval(() => {
    if (hovered.value && props.config.hoverPause) {
      return;
    }

    if (props.config.data.length <= (props.config.rowNum || 5)) {
      return;
    }
