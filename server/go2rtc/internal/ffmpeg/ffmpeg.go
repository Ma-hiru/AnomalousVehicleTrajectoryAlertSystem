...
import (
	...
	"server/go2rtc/internal/ffmpeg/device"
	"server/go2rtc/internal/ffmpeg/hardware"
	"server/go2rtc/internal/ffmpeg/virtual"
	"server/go2rtc/internal/rtsp"
	"server/go2rtc/internal/streams"
	"server/go2rtc/pkg/core"
	"server/go2rtc/pkg/ffmpeg"
)
...
var defaults = map[string]string{
	...
	// hardware Rockchip
	// important to use custom ffmpeg https://server/go2rtc/issues/768
...
func parseArgs(s string) *ffmpeg.Args {
	...
	if query != nil {
		...
		for _, raw := range query["raw"] {
			// support templates https://server/go2rtc/issues/487
