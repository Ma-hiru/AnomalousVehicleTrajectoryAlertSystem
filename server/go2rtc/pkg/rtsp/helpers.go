...
func UnmarshalSDP(rawSDP []byte) ([]*core.Media, error) {
	...
	if err := sd.Unmarshal(rawSDP); err != nil {
		...
		// fix broken `c=` https://server/go2rtc/issues/1426
		rawSDP = regexp.MustCompile("\nc=[^\n]+").ReplaceAll(rawSDP, nil)
	...
	// fix buggy camera https://server/go2rtc/issues/771
...
func urlParse(rawURL string) (*url.URL, error) {
	// fix https://server/go2rtc/issues/830
