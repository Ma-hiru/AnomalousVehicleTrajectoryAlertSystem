...
func newStream(db *gorm.DB, opts ...gen.DOOption) stream {
	...
	_stream.StreamID = field.NewInt32(tableName, "streamId")
	_stream.StreamName = field.NewString(tableName, "streamName")
	_stream.Addr = field.NewString(tableName, "addr")
	_stream.Latitude = field.NewFloat64(tableName, "latitude")
	_stream.Longitude = field.NewFloat64(tableName, "longitude")
...
func (s *stream) updateTableName(table string) *stream {
	...
	s.StreamID = field.NewInt32(table, "streamId")
	s.StreamName = field.NewString(table, "streamName")
...
func (s *stream) WithContext(ctx context.Context) IStreamDo { return s.streamDo.WithContext(ctx) }

func (s stream) TableName() string { return s.streamDo.TableName() }

func (s stream) Alias() string { return s.streamDo.Alias() }

func (s stream) Columns(cols ...field.Expr) gen.Columns { return s.streamDo.Columns(cols...) }
...
func (s *stream) fillFieldMap() {
	...
	s.fieldMap["streamId"] = s.StreamID
	s.fieldMap["streamName"] = s.StreamName
...
type IStreamDo interface {
	...
	Unscoped() IStreamDo
	Create(values ...*model.Stream) error
	CreateInBatches(values []*model.Stream, batchSize int) error
	Save(values ...*model.Stream) error
	First() (*model.Stream, error)
	Take() (*model.Stream, error)
	Last() (*model.Stream, error)
	Find() ([]*model.Stream, error)
	FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*model.Stream, err error)
	FindInBatches(result *[]*model.Stream, batchSize int, fc func(tx gen.Dao, batch int) error) error
	Pluck(column field.Expr, dest interface{}) error
	Delete(...*model.Stream) (info gen.ResultInfo, err error)
	...
	FirstOrInit() (*model.Stream, error)
	FirstOrCreate() (*model.Stream, error)
	FindByPage(offset int, limit int) (result []*model.Stream, count int64, err error)
